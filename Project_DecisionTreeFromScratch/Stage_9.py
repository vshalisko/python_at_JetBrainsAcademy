...
class DecisionTree:
    ...
    def __init__(self, node_minimum = 1, verbose = False):
    ...
    def _split(self, data, target):
        ...
        for feature in features:
            if feature in self.numerical_features:
                ...
                breaks = sorted(data[feature].unique(), reverse=True)
                for value in breaks:
    ...
    def _recursive_prediction(self, node, data):
        if node.term:
            ...
            if node.feature in self.numerical_features:
                left_check = data[node.feature] <= node.value
            else:
                left_check = data[node.feature] == node.value
            if left_check:
...
if __name__ == '__main__':
    ...
    train_data_path = "test/data_stage9_train - copia.csv"
    train_data = pd.read_csv(train_data_path, index_col=0)
    test_data_path = "test/data_stage9_test - copia.csv"
    ...
    dt = DecisionTree(node_minimum = 74)
