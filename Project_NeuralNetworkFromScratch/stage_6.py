...
class TwoLayerNeural():
    ...
    def backprop(self, X, y, alpha):
        ...
        result_network = self.forward(X)
        # Perform the backward pass
        error2 = ((result_network - y) *
                  sigmoid_derivative(sigmoid(np.dot(self.layer1, self.W2) + self.b2)))
        delta_W2 = np.dot(self.layer1.T, error2)
        delta_b2 = np.sum(error2, axis=0, keepdims=True)
        error1 = (np.dot(error2, self.W2.T) *
                  sigmoid_derivative(sigmoid(np.dot(X, self.W1) + self.b1)))
...
if __name__ == '__main__':
    ...
    NN2.backprop(X_train[0:2, :], y_train[0:2], alpha=0.1)
    r4 = NN2.forward(X_train[0:2, :])
    #print(r3.flatten().tolist())
    #print(r4.flatten().tolist())

    print(mse(r4, y_train[0:2]).flatten().tolist())

    #true_backprop_res = [0.17205149953305518]
    #print(true_backprop_res)
