...
if __name__ == '__main__':
    ...
    for k in k_range:
        ...
        silhouette_value = silhouette_score(X_full, y)
        silhouette.append(silhouette_value)

    max_value = max(silhouette)
    max_index = silhouette.index(max_value)

    k_model = CustomKMeans(k=k_range[max_index])
    if not k_model.centers:
        k_model.centers = X_full[0:k_model.k]
    k_model.fit(X_full)
    y = k_model.predict(X_full)

    plot_comparison(X_full, y, y_full)

    cs = ', '.join(map(str, y[:20]))
    print(f"[{cs}]")
