...
def features_probability_calculator(data_set):
    ...
    feature_df['Spam Probability'] = (feature_df['N_spam'] + 1) / (spam_world_counter + total_world_counter)
    feature_df['Ham Probability'] = (feature_df['N_ham'] + 1) / (ham_world_counter + total_world_counter)
...
def main():
    ...
    random_set = data.sample(frac=1, random_state=43, ignore_index=False)
    train_last_row = int(random_set.shape[0] * 0.8)
    train_set = random_set[0:train_last_row]
    test_set = random_set[train_last_row:]
    ...
    train_feature_df = features_probability_calculator(train_set)
    #print(train_feature_df.iloc[:200, :])

    N_spam = train_set.loc[train_set['Target'] == "spam", :].shape[0]
    N_ham = train_set.loc[train_set['Target'] == "ham", :].shape[0]
    ...
    for ind in test_set.index:
        ...
        new_row = pd.DataFrame({'Predicted': [predicted],
                                'Actual': [test_set['Target'][ind]]},
                               index=[ind])
        classification_df = pd.concat([classification_df, new_row])
    print(classification_df.iloc[:50, :])
